...
def process_emotional_tone(data_path):
    ...
    clean_emotional_tone = clean_emotional_tone['Эмоциональный_тон_LLM'].replace(to_replace=[None], value=np.nan)
    ...
    if 1 in clean_emotional_tone.values:
        positive_tone_count = clean_emotional_tone[clean_emotional_tone['Эмоциональный_тон_LLM'].astype(str).str.contains('1')].groupby('Час старта урока').size()
...
process_emotional_tone('analis.csv')